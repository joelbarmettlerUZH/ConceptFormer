...
from src.Datasets.factory import rex_factory, web_qsp_factory
...
parser.add_argument('--dataset_name', type=str, default='TRExBite',
                    help='Name of the dataset to evaluate (TriREx, TriRExLite, TRExBite, TRExBiteLite or WebQSP)')
parser.add_argument('--gpu_indices', nargs='*', type=int, default=[2],
...
def main(config_index, number_of_neighbors, gpu):
    device = torch.device(f"cuda:{gpu}" if torch.cuda.is_available() and gpu > -1 else "cpu")

    if DATASET_NAME == "WebQSP":
        sentence_test_data, graphs = web_qsp_factory()
    else:
        _, _, sentence_test_data, graphs,  = rex_factory(DATASET_NAME)

    injection_format = "Subject, {predicate_1} {object_1}, {predicate_2} {object_2}, ..."

    llm = configs[config_index](device)

    wandb.init(
        project="7_eval_basemodel_textinjection",
        group="gpt-2" if llm.name.startswith("gpt") else "llama-2",
        name=f"{llm.model_name_or_path}-{number_of_neighbors}-neighbours",
    ...
    for i in tqdm(range(200_000), desc=f'Evaluating', unit='sentences'):
        data = sentence_test_data[i]

        sentence = data['sentence']
        strived_for_k = data.get('k', 1)
        _subject_id = data['subject']['id']
        _subject = data['subject']['label']
        subject_start, subject_end = data['subject']['boundaries']

        _predicate = data['predicate']['label']

        _object = data['object']['label']
        _object_id = data['object']['id']
        object_start = data['object']['boundaries'][0]

        if _subject_id not in graphs:
            continue

        G = graphs[_subject_id]

        node_labels = []
        edge_labels = []

        for central_node_id, neighbour_node_id, edge in G.edges(data=True):
            if neighbour_node_id == _object_id:
                continue
        ...
        node_edge_zip = list(zip(node_labels, edge_labels))
        node_edge_zip = random.sample(node_edge_zip, min(len(node_edge_zip), number_of_neighbors - 1))
        node_edge_zip.append((_object, _predicate))
        ...
        injection = f"{reduced_edge_attrs[0]} {reduced_neighbor_nodes[0]}"
        for node, edge in list(zip(reduced_neighbor_nodes, reduced_edge_attrs))[1:]:
            injection = f"{injection}, {edge} {node}"

        source_text = f"{source_text[:subject_end].rstrip()}, {injection}, {source_text[subject_end:].lstrip()}"
        injection_len = len(llm.tokenizer.encode(source_text))
        source_text_len = len(llm.tokenizer.encode(injection))

        if source_text_len > llm.max_sequence_length:
            total_context_exceeded += 1
            continue

        if not (source_text and target_text):
            continue

        logging.debug(f"{source_text}|{target_text}")

        embeddings = llm.early_embedding(source_text)
        try:
            top_k_output = llm.predict_top_k_sequence(embeddings, source_text=source_text, target_text=target_text,
                                                      k=k)
        ...
        total_tokens_injected += injection_len

    x_values = list(range(1, k + 1))
    y_values = list([hits[i] / (hits[i] + misses[i] + 1e-8) for i in range(k)])
    ...
    wandb.summary['n_sentences'] = hits[0] + misses[0]
    wandb.summary[f'strived_for_k_hit_ratio'] = total_strived_for_k_hits / (hits[0] + misses[0])

    for k_i in [1, 2, 3, 4, 5, 10, 15, 25, 50]:
        wandb.summary[f'k{k_i}'] = hits[k_i - 1] / (hits[k_i - 1] + misses[k_i - 1] + 1e-8)

    wandb.summary['avg_tokens_injected'] = (total_tokens_injected / (hits[0] + misses[0] + 1e-8))
    wandb.summary['total_error_context_exceeded'] = total_context_exceeded
    wandb.summary['ratio_error_context_exceeded'] = (total_context_exceeded / (hits[0] + misses[0] + 1e-8))
    wandb.finish()
...
def process_function(config_indices_queue, gpu):
    while True:
        try:
            config_index, number_of_neighbours = config_indices_queue.get_nowait()
        except multiprocessing.queues.Empty:
            break

        print(f"Processing {DATASET_NAME} on device {gpu}")
        main(config_index, number_of_neighbours, gpu)
...
if __name__ == "__main__":
    ...
    for config_index in range(len(configs)):
        for number_of_neighbours in [100]:
            config_indices_queue.put((config_index, number_of_neighbours))
