...
\begin{table}[t]
  \caption{Zero-shot transfer to MetaQA-1hop (full test set, $n{=}9{,}947$; Wilson 95\% CIs).
  The Wikidata-trained checkpoints of Table~\ref{tab:kfamily} (frozen Qwen3-0.6B, 100k corpus;
  one seed per $k$) are evaluated unchanged on a movie KG they have never seen --- no MetaQA
  training, no adaptation, no shared entities or relations. RAG is budgeted verbalization of
  the same neighborhoods (no answer guarantee).}
  \label{tab:transfer}
  \centering
  \small
  \begin{tabular}{lcccccc}
    \toprule
    & base & $k{=}1$ & $k{=}8$ & $k{=}16$ & $k{=}32$ & RAG\\
    \midrule
    accuracy & .058 & .161 & .235 & .244 & \textbf{.335} & .921\\
    95\% CI & [.053, .062] & [.153, .168] & [.227, .244] & [.235, .252] & [.325, .344] & [.916, .926]\\
    \bottomrule
  \end{tabular}
\end{table}
...
$p<10^{-200}$ at every $k$). The curve remains monotone in $k$, and the shape mirrors the
home-graph curve: the $k{=}16 \to k{=}32$ step alone adds $9.1$\,pt ($p<10^{-114}$).
Measured as relative gap closure, zero-shot transfer retains roughly half to two-thirds of
the home-graph effect (21\% vs.\ 44\% at $k{=}8$; 32\% vs.\ 51\% at $k{=}32$). The RAG
...
cross-backbone comparisons involve each model's own curriculum; augmentation's effect is
