...
\begin{abstract}
  ...
  across the Qwen3 family the injected margin shrinks as the frozen model grows --- with the
  same data step buying the 1.7B model half the relative gain it buys the 0.6B.
...
\begin{table}[t]
  ...
  \begin{tabular}{llllll}
    ...
    Qwen3-1.7B & .141 & $.203{\pm}.003$ (+6.2) & $.253$ (+11.2) & $1.8\times$ (margin) & 3 / 1\\
    Qwen3-4B & .154 & $.234$ (+8.0) & pending & --- & 1 / ---\\
    \bottomrule
  \end{tabular}
\end{table}
...
only $1.8\times$ --- at 100k entities the 1.7B (+11.2\,pt) barely reaches what the 0.6B
already had at 10k (+10.7\,pt). Headroom does not explain this: base accuracy moves only
$0.10 \to 0.15$ across the family while the RAG ceiling stays at $\sim$0.97, yet relative gap
closure at 100k falls from 44\% (0.6B) to 14\% (1.7B). Two further observations sharpen the
picture. First, the 1.7B's \emph{held-out} accuracy is flat across the data step
($0.397 \to 0.403$) where the 0.6B's rises ($0.416 \to 0.545$): the encoder trains equally
well on the bigger backbone; what shrinks is specifically the unseen-entity payoff. Second,
the axis is not monotone --- the 4B's 10k margin (+8.0\,pt) sits \emph{above} the 1.7B's
(+6.2\,pt, seed spread $\pm$0.3), a partial recovery consistent with larger models being
better consumers of injected context, though the teacher-relative curricula (a stronger base
tiers a different question mix into training) remain a candidate confound.
...
family, yet the $10\times$ data step buys the 1.7B half the relative improvement it buys the
0.6B (\S\ref{sec:scaling}). The 4B's partial recovery at 10k keeps the axis from being a
